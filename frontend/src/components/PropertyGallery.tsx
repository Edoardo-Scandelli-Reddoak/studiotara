...
<div id="brochure-content" style="font-family:'Helvetica Neue',Arial,sans-serif;color:#1a1a1a;line-height:1.6;padding:0">
  ...
  <div style="margin-top:28px;padding-top:14px;border-top:2px solid #092d74;text-align:center;color:#888;font-size:11px">
    ...
    Tel: 02 3655 7365 — info@studiotara.it — www.studiotara.it
  </div>
</div>`;
...
<script>window.onload=function(){window.print()}</script></body></html>`;
  ...
  return (
    <>
      {/* Gallery */}
      {images.length > 0 ? (
        <div className="relative w-full aspect-[4/3] rounded-[12px] md:rounded-[20px] overflow-hidden mt-4 md:mt-5 group">
          <Image
          ...
          {images.length > 1 && (
            <>
              <button
                ...
                className="absolute left-2 md:left-3 top-1/2 -translate-y-1/2 w-[44px] h-[44px] md:w-[40px] md:h-[40px] bg-white/95 md:bg-white/90 md:hover:bg-white rounded-full flex items-center justify-center shadow-md opacity-100 lg:opacity-0 lg:group-hover:opacity-100 md:transition-opacity cursor-pointer active:scale-95"
              >
                <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="#092d74" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round">
              ...
              <button
                ...
                className="absolute right-2 md:right-3 top-1/2 -translate-y-1/2 w-[44px] h-[44px] md:w-[40px] md:h-[40px] bg-white/95 md:bg-white/90 md:hover:bg-white rounded-full flex items-center justify-center shadow-md opacity-100 lg:opacity-0 lg:group-hover:opacity-100 md:transition-opacity cursor-pointer active:scale-95"
              >
                <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="#092d74" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round">
          ...
          {images.length > 1 && (
            <div className="absolute bottom-3 md:bottom-4 left-1/2 -translate-x-1/2 flex gap-1.5 md:gap-2 max-w-[60%] overflow-hidden">
              {images.slice(0, 8).map((_, i) => (
                <button
                  key={i}
                  onClick={() => setCurrentIndex(i)}
                  aria-label={`Vai alla foto ${i + 1}`}
                  className={`w-[7px] h-[7px] md:w-[8px] md:h-[8px] rounded-full md:transition-all cursor-pointer ${
                    i === currentIndex ? "bg-white scale-125" : "bg-white/50"
                  }`}
                />
              ))}
              {images.length > 8 && (
                <span className="text-white/80 text-[10px] ml-1">+{images.length - 8}</span>
              )}
            </div>
          )}
        </div>
      ) : (
        <div className="w-full aspect-[4/3] rounded-[12px] md:rounded-[20px] overflow-hidden mt-4 md:mt-5 bg-[#f5f3f0] flex items-center justify-center">
          <span className="text-black/30 text-[14px] md:text-[15px]">Nessuna immagine disponibile</span>
        </div>
      )}

      {/* Tabs */}
      <div className="flex gap-2 mt-3 overflow-x-auto scrollbar-hide -mx-4 px-4 md:mx-0 md:px-0 md:overflow-x-visible">
        {/* Foto */}
        <button className="shrink-0 px-3.5 md:px-4 py-2 rounded-[8px] md:rounded-[6px] text-[13px] md:text-[14px] font-medium bg-blue-primary text-white flex items-center gap-1.5">
        ...
        <button
          ...
          className={`shrink-0 px-3.5 md:px-4 py-2 rounded-[8px] md:rounded-[6px] text-[13px] md:text-[14px] font-medium flex items-center gap-1.5 md:transition-all ${
            hasVideo
              ? "bg-[#f5f3f0] text-black/70 md:hover:bg-[#eae8e5] cursor-pointer active:bg-[#eae8e5]"
        ...
        <button
          ...
          disabled={generating}
          className={`shrink-0 px-3.5 md:px-4 py-2 rounded-[8px] md:rounded-[6px] text-[13px] md:text-[14px] font-medium md:transition-all cursor-pointer flex items-center gap-1.5 ${
            generating
              ? "bg-blue-primary text-white"
              : "bg-[#f5f3f0] text-black/70 md:hover:bg-[#eae8e5] active:bg-[#eae8e5]"
          }`}
        >
          {generating ? (
