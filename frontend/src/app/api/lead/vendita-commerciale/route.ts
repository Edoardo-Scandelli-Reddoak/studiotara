...
// Pipeline & stage for "SITO - richiesta vendita commerciale"
const PIPELINE_ID = "bb426479-e671-482e-8d58-c66b0b82447b";
const STAGE_ID = "823c7a7e-b9b4-499e-a0e3-20cb4a2d8950";
const SOURCE_ID = 1;
...
export async function POST(req: Request) {
  ...
  if (!contactRes.ok) {
    ...
    console.error("CRM contact create failed (vendita-commerciale):", contactRes.status, errText);
    return NextResponse.json(
  ...
  if (!dealRes.ok) {
    ...
    console.error("CRM deal create failed (vendita-commerciale):", dealRes.status, errText);
    return NextResponse.json(
