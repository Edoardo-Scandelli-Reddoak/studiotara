...
// Pipeline & stage for "SITO - ricerca immobile ideale (residenziale)"
const PIPELINE_ID = "1bd23036-8b54-4c8e-a99d-906d6e25db46";
const STAGE_ID = "5b6c53ce-56cc-496a-b251-06ab73556ad4";
const SOURCE_ID = 1;
...
export async function POST(req: Request) {
  ...
  if (!contactRes.ok) {
    ...
    console.error("CRM contact create failed (ricerca-residenziale):", contactRes.status, errText);
    return NextResponse.json(
  ...
  if (!dealRes.ok) {
    ...
    console.error("CRM deal create failed (ricerca-residenziale):", dealRes.status, errText);
    return NextResponse.json(
