...
class Article(models.Model):
    ...
    contenuto = HTMLField(verbose_name='Contenuto')
    excerpt = models.TextField(blank=True, verbose_name='Riassunto')
    immagine = models.ImageField(
        upload_to='blog/', null=True, blank=True,
        verbose_name='Immagine copertina (pagina articolo)',
        help_text='Banner mostrato in cima alla pagina del singolo articolo.',
    )
    immagine_card = models.ImageField(
        upload_to='blog/', null=True, blank=True,
        verbose_name='Immagine card (lista blog)',
        help_text='Miniatura della card nella lista degli articoli. Se vuota, viene usata la "Immagine copertina".',
    )
